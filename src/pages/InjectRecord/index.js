...
// ]
let data,columns;
class InjectRecord extends Component{ 
    state={
         message:'您好',
         columns:columns,
         data:data
     ...
     componentWillMount(){
        console.log('组件即将被挂载');
        axios.get('http://rap2api.taobao.org/app/mock/225049/injManage',{
            params:{
                cursor:1
            }
        }).then(res=>{
             data=res.ijArr.map(item=>({...item,key:item.ID}));
             columns=res.ijTree.map(({prop,label})=>({
                title:label,
                dataIndex:prop,
                key:prop
         }));
